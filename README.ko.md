...
개인 채팅에서만 사용할 수 있습니다. 한 번에 하나의 로그인만 진행할 수 있고 시도 후 10분의 대기 시간이 적용됩니다. 인증 안내와 화면에 바로 보이는 복사하기 편한 코드 전용 메시지는 별도로 전송되며 둘 다 콘텐츠 보호가 활성화됩니다. 두 메시지는 성공·실패 또는 10분 제한 시간 경과 후 삭제됩니다.
...
- 텔레그램 `/login`은 명시적 확인이 필요하고 개인 채팅에서만 작동합니다. 로그인 시도 횟수를 제한하고, 코드를 별도 메시지로 보내며, 텔레그램 콘텐츠 보호를 활성화하고, 절차가 끝나면 로그인 메시지를 삭제합니다.
